...
### Fixed
- CI перестал быть декоративным. У job с проверками стоял
  `continue-on-error: true`, а у сборки Docker-образа — `if: always()`, из-за
  чего workflow не мог стать красным ни при падении тестов, ни при
  недостижении порога покрытия 90%. Обещанные в README `ruff` и `mypy` в
  workflow вообще отсутствовали, как и вызов `scripts/check_deps_sync.py`.
  Теперь шаги совпадают с целями Makefile (`make lint`, `make test`), так что
  локальный прогон и CI проверяют одно и то же.
